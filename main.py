...
board_state = [[' ' for _ in range(cols)] for _ in range(rows)]  # setting up board
                ...
                for index, area in enumerate(clickable_areas):   # identify which cell is clickable
                    ...
                    if area.collidepoint(event.pos):  # whether a cell is clicked
                        ...
                        if board_state[row][col] == ' ':  # used to check if this cell is empty
                            ...
                            if rows in board_state[row] == x_img:
                                player1_victory = True
                                if player1_victory == True:
                                    victory_1 = my_font.render("Player 1 Victory!", True, (251, 172, 193))
                                    screen.blit(victory_1, (100, 100))
